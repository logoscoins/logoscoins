...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("LogosCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LogosCoinTEST2\r");
            Send(hSocket, "WHO #LogosCoinTEST2\r");
        } else {
            // randomly join #LogosCoin00-#LogosCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #LogosCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #LogosCoin%02d\r", channel_number).c_str());
        }

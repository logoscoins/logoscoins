LogosCoin-qt: Qt4 GUI for LogosCoin
===============================
...
Alternatively, install Qt Creator and open the `LogosCoin-qt.pro` file.

An executable named `LogosCoin-qt` will be built.
...
A warning for people using the *static binary* version of LogosCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of LogosCoin is linked against libdb5.3.
...
installed causes LogosCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
